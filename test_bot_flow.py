...
def test_poll_rejection_falls_back_and_never_crashes():
    async def t():
        async with Harness() as h:
            ...
            assert h.all_text(1500).count("प्रश्न संख्या 1?") == 1          # …exactly once (no duplicate)
            await h.answer_current(u, True)
...
# ============================================================ round 2 flows
def test_import_needing_review_requires_the_review_button_and_plays_5_options():
    async def t():
        async with Harness() as h:
            u = h.f.user(1201)
            await h.cb(u, "m:imp")
            ...
            txt = ("1. पाँच विकल्प?\n" + opts5 + "उत्तर: E\n"
                   ...
                   "3. तीन विकल्प?\n(A) x\n(B) y\n(C) z\nउत्तर: C\n")
            await h.text(u, txt)
            last = h.api.sent("sendMessage", 1201)[-1]
            assert "जांचें" in last["text"] and "प्रश्न 3 (3)" in last["text"]
            assert "(E) इनमें से कोई नहीं ✅" in last["text"]            # preview shows E, marked correct
            cbs = [b.get("callback_data") for b in buttons(last["reply_markup"])]
            assert "i:reviewed" in cbs and "i:create" not in cbs
            await h.cb(u, "i:create")                                   # forged old button: refused
            assert db.get_owner_quizzes(1201) == []
            await h.cb(u, "i:reviewed")
            await h.cb(u, "i:usedefault")
            qz = db.get_owner_quizzes(1201)
            assert len(qz) == 1
            ...
            for i in range(3):
                await h.wait(lambda: len(h.polls_for(1201)) == i + 1)
                await h.answer_current(u, True)
            await h.wait(lambda: "🏁" in h.all_text(1201))
            assert "Correct: 3" in h.all_text(1201)
    run(t())


def test_twelve_long_options_use_compact_labels_and_never_break_limits():
    async def t():
        async with Harness() as h:
            db.ensure_user_row(1301)
            qid = db.new_quiz(1301, "Twelve", "", timer=30, shuffle_options=1)
            ...
            texts = h.all_text(1301)
            for o in opts:
                assert o.strip() in texts                                 # full original text sent
            # the compact label chosen as correct really maps to option 12
            k = poll["correct_option_id"]
            label = "ABCDEFGHIJKL"[k]
            assert f"({label}) {opts[11].strip()}" in texts
            await h.answer_current(u, True)
            await h.wait(lambda: "🏁" in h.all_text(1301))
            assert "Correct: 1" in h.all_text(1301)
    run(t())


def test_manual_wizard_two_options():
    async def t():
        async with Harness() as h:
            u = h.f.user(1401)
            await h.cb(u, "m:new")
...
def test_scanned_pdf_document_import_with_ocr_progress(tmp_path):
    ...
    G.make_scanned_pdf(pdf, "प्रश्न 1. राजस्थान की राजधानी क्या है?\n(A) जोधपुर\n(B) जयपुर\n(C) उदयपुर\n"
                            "(D) अजमेर\nउत्तर: (B)\nप्रश्न 2. Which is the state animal?\n(A) Camel\n"
                            "(B) Tiger\n(C) Lion\n(D) Deer\nAnswer: (A)", dpi=300)

    async def t():
        async with Harness() as h:
            u = h.f.user(1501)
            h.api.files["SCAN1"] = pdf.read_bytes()
            await h.send(h.f.document(u, "SCAN1", "scan.pdf", "application/pdf", pdf.stat().st_size))
            out = h.all_text(1501)
            assert "OCR" in out and "राजस्थान की राजधानी क्या है?" in out
            last = h.api.sent("sendMessage", 1501)[-1]
            assert "i:reviewed" in str(last["reply_markup"])                # OCR → confirmation
            await h.cb(u, "i:reviewed")
            await h.cb(u, "i:usedefault")
            qs = db.get_questions(db.get_owner_quizzes(1501)[0]["id"])
            assert [x["options"][x["correct_index"]] for x in qs] == ["जयपुर", "Camel"]
    run(t())


def test_production_poll_limits_utf16_multiline_and_2_3_5_12_options():
    """Telegram counts UTF-16 units: '𝑥' / emoji = 2.  Nothing may ever be
    rejected by Telegram, and every original word must reach the chat."""
    async def t():
        async with Harness() as h:
            db.ensure_user_row(1601)
            qid = db.new_quiz(1601, "Limits", "", timer=0)
            cases = [
                # 60 × '𝑥' = 60 code points but 120 UTF-16 units → must NOT be sent natively
                ("गणित: सही व्यंजक चुनें?", ["𝑥" * 60, "𝑦", "𝑧"], 0),
                # 160 emoji question = 320 UTF-16 units > 300; two options
                ("🙂" * 160 + " सही?", ["सत्य", "असत्य"], 1),
                # multi-line option (poll options are single-line)
                ("बहु-पंक्ति विकल्प?", ["पहली पंक्ति\nदूसरी पंक्ति", "b", "c", "d", "(E) अनुत्तरित प्रश्न"], 4),
                # 12 short options — native, no compaction
                ("बारह छोटे विकल्प?", [f"विकल्प {i}" for i in range(12)], 7),
                # exactly 100 UTF-16 units: 50 × '𝑥' → allowed natively
                ("ठीक 100 units?", ["𝑥" * 50, "b"], 0),
            ]
            ...
            for n in range(1, len(cases) + 1):
                ...
                await h.answer_current(u, True)
            await h.wait(lambda: "🏁" in h.all_text(1601))
            assert h.api.rejections == [], h.api.rejections                 # Telegram never said no
            polls = [p for _, p in h.polls_for(1601)]
            texts = h.all_text(1601)
            assert [len(p["options"]) for p in polls] == [3, 2, 5, 12, 2]
            assert polls[0]["options"] == ["(A)", "(B)", "(C)"] and "𝑥" * 60 in texts
            assert "🙂" * 160 in texts                                       # long question in a message
            assert "पहली पंक्ति\nदूसरी पंक्ति" in texts                    # multi-line kept verbatim
            assert "(E) अनुत्तरित प्रश्न" in texts and len(polls[2]["options"]) == 5
            assert polls[3]["options"] == [f"विकल्प {i}" for i in range(12)]  # 12 native options
            assert polls[4]["options"] == ["𝑥" * 50, "b"]
            assert "Correct: 5" in texts
    run(t())
