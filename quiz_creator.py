...
async def _prompt_options(update: Update) -> None:
    await update.effective_chat.send_message(
        "5️⃣ <b>Step 5:</b> ठीक <b>4 options</b> भेजें — हर line में एक option:\n\n"
        "<code>(A) पहला विकल्प\n(B) दूसरा विकल्प\n(C) तीसरा विकल्प\n(D) चौथा विकल्प</code>\n\n"
...
async def handle_creator_message(update: Update, context, state: str, data: dict) -> None:
    ...
    if state == "c_q":
        ...
        if detected:
            ...
            preview = "\n".join(f"({config.OPTION_LABELS[i]}) {esc(o)}" for i, o in enumerate(detected[1]))
            await msg.reply_text(
                f"🔎 आपके text में ये 4 options मिले:\n\n{preview}\n\nक्या इन्हें options मानें?"[:config.MESSAGE_MAX],
                parse_mode=ParseMode.HTML,
                reply_markup=M([[B("✅ हाँ, ये options हैं", callback_data="c:useopts")],
    ...
    if state == "c_correct":
        await msg.reply_text("👆 ऊपर के buttons से सही उत्तर (A/B/C/D) चुनें।")
        return
...
IMPORT_HELP = (
    "📥 <b>Import PDF/Text</b>\n\n"
    "Selectable-text <b>PDF</b>, <b>.txt</b> file, या सीधे <b>text</b> भेजें।\n\n"
    "• प्रश्न: <code>प्रश्न 1.</code> / <code>1.</code> / <code>1)</code> / <code>Q1</code> / <code>Q.1</code>\n"
    "• Options: <code>(A) … (D)</code> (matching/assertion/ordering data के बाद आने वाले असली options पहचाने जाते हैं)\n"
...
async def handle_import_document(update: Update, context) -> None:
    ...
    try:
        ...
        text = pdf_parser.extract_pdf_text(raw) if is_pdf else pdf_parser.decode_text_bytes(raw)
    ...
    await _parse_and_review(update, user.id, text, default_title, status_msg=status)
...
async def _parse_and_review(update: Update, user_id: int, text: str, default_title: str,
    ...
    head += [f"✅ सही parse हुए प्रश्न: <b>{n}</b>", f"❌ समस्या वाले प्रश्न: <b>{ne}</b>"]
    if types:
    ...
    if n:
        q0 = questions[0]
        prev = q0["question"][:300] + ("…" if len(q0["question"]) > 300 else "")
        head += ["", f"👁 <b>Preview (प्रश्न {q0['number']})</b>:", esc(prev)]
        for i, o in enumerate(q0["options"]):
            mark = " ✅" if i == q0["correct_index"] else ""
            head.append(f"({config.OPTION_LABELS[i]}) {esc(o[:150])}{mark}")

    rows = []
    if n and not ne:
        if n <= config.MAX_QUESTIONS:
            rows.append([B(f"✅ Quiz बनाएँ ({n} प्रश्न)", callback_data="i:create")])
        else:
            k = -(-n // config.MAX_QUESTIONS)
            rows.append([B(f"✂️ {k} quizzes में बाँटें (≤{config.MAX_QUESTIONS} each)", callback_data="i:split")])
    elif n and ne:
        head += ["", f"⚠️ {ne} प्रश्न parse नहीं हुए। Partial quiz तभी बनेगा जब आप नीचे confirm करें।"]
        if n <= config.MAX_QUESTIONS:
    ...
    for i, chunk in enumerate(engine.split_message(summary, 3800)):
        last = i == len(engine.split_message(summary, 3800)) - 1
        await chat.send_message(chunk, parse_mode=ParseMode.HTML, reply_markup=M(rows) if last else None)
...
async def handle_import_callback(update: Update, context) -> None:
    ...
    if state not in ("imp_review", "imp_title"):
        ...
        return
    await q.answer()
    if action in ("create", "partial", "split", "partialsplit"):
        if not data.get("questions"):
            await update.effective_chat.send_message("❌ कोई valid प्रश्न नहीं है।")
            return
        data["split"] = action in ("split", "partialsplit")
