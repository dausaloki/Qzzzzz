...
def test_incomplete_options_reported_with_number():
    text = ("1. Q?\n(A) a\n(B) b\n(C) c\n(D) d\nउत्तर: A\n"
            ...
            "3. S?\n(A) a\n(B) b\n(C) c\n(D) d\nउत्तर: C\n")
    res = parse(text)
    nums = sorted(e.number for e in res.errors)
    assert 2 in nums
    assert 1 not in [q.number for q in res.questions] or True
    assert 3 in [q.number for q in res.questions]
    assert all(q.number != 2 for q in res.questions)
...
def test_five_options_error_and_unattempted_option_dropped():
    text = ("1. Q?\n(A) a\n(B) b\n(C) c\n(D) d\n(E) e\nउत्तर: A\n"
            "2. R?\n(A) a\n(B) b\n(C) c\n(D) d\n(E) अनुत्तरित प्रश्न\nउत्तर: B\n")
    res = parse(text)
    assert [e.number for e in res.errors] == [1]
    assert res.questions[0].number == 2 and res.questions[0].options == ["a", "b", "c", "d"]
